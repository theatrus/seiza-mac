...
[**Download Seiza for Mac 0.4.1**](https://github.com/theatrus/seiza-mac/releases/latest/download/Seiza-0.4.1-universal.dmg) · [Release notes and other downloads](https://github.com/theatrus/seiza-mac/releases/latest)
...
| Directory stacking | Aligns and combines selected FITS or XISF frames from an open folder. When names show two or more filters such as L/R/G/B, Ha, OIII, S/SII, or H-beta, Seiza can save one stack per filter in a folder; otherwise it saves one file. Choose references, normalization, sample rejection, registration limits, and optional bias, dark, and flat masters; then follow progress, cancel between frames, and open the saved 32-bit floating-point FITS results. |
